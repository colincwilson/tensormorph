...
def randvecs_sim2(n, dim, dpMatrix, lb=-1.0, ub=1.0, scale=True):
    M = np.matrix(np.random.rand(dim,
                                 n))  # xxx respect bounds at initialization
    step0 = 0.1
    tol = 1e-6
    maxIts = 50000
    for i in range(maxIts):
        ...
        M = M - step * inc
        M[M < lb] = lb
        M[M > ub] = ub  # xxx bound values to [lb,ub]
        ...
        if maxDiff <= tol:
            ...
            return M
    print(
        'randvecs_sim2: Failed to find solution to tolerance in specified iterations'
    )
    return M
